...
contract AgentExecutionTest is Test {
    ...
    function test_LogExecutionStart_EmitsEvent() public {
        ...
        emit AgentExecution.ExecutionStarted(1, agentId1, user1, 1000, tokenIn, tokenOut);

        execution.logExecutionStart(user1, 1000, tokenIn, tokenOut);
    }

    function testFail_LogExecutionStart_NotRegisteredAgent() public {
        vm.prank(address(0x999));
        execution.logExecutionStart(user1, 1000, tokenIn, tokenOut);
    }

    function testFail_LogExecutionStart_InvalidUser() public {
        vm.prank(agentWallet1);
        execution.logExecutionStart(address(0), 1000, tokenIn, tokenOut);
    }

    function testFail_LogExecutionStart_ZeroAmount() public {
        vm.prank(agentWallet1);
    ...
    function test_LogExecutionComplete_EmitsEvent() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit AgentExecution.ExecutionCompleted(
            executionId,
            agentId1,
            user1,
            1000,
    ...
    function testFail_LogExecutionComplete_NotExecutionOwner() public {
        vm.prank(agentWallet1);
        uint256 executionId = execution.logExecutionStart(user1, 1000, tokenIn, tokenOut);
    ...
    function testFail_LogExecutionComplete_NotPending() public {
        vm.startPrank(agentWallet1);
    ...
    function test_LogRedelegation_EmitsEvent() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit AgentExecution.RedelegationCreated(agentId1, agentId2, user1, 500, 3600);

        execution.logRedelegation(agentId2, user1, 500, 3600);
    }

    function testFail_LogRedelegation_NotRegisteredAgent() public {
        vm.prank(address(0x999));
        execution.logRedelegation(agentId2, user1, 500, 3600);
    }

    function testFail_LogRedelegation_SelfDelegation() public {
        vm.prank(agentWallet1);
        execution.logRedelegation(agentId1, user1, 500, 3600);
    }

    function testFail_LogRedelegation_ZeroAmount() public {
        vm.prank(agentWallet1);
        execution.logRedelegation(agentId2, user1, 0, 3600);
    }

    function testFail_LogRedelegation_ZeroDuration() public {
        vm.prank(agentWallet1);
